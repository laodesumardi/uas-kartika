...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        document.getElementById("refresh-captcha").addEventListener("click", function (event) {
            ...
            fetch('{{ route("reload.captcha") }}') // Menggunakan route named untuk reload CAPTCHA
                .then(response => response.json())
                .then(data => {
                    document.getElementById('captcha-img').src = data.captcha + '?' + new Date().getTime(); // Menambahkan timestamp untuk mencegah caching
                })
